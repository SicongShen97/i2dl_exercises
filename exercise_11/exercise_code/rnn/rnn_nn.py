...
class LSTM(nn.Module):
    def __init__(self, input_size=1, hidden_size=20):
        ...
        self.Wf = nn.Linear(self.input_size, self.hidden_size, bias=True)
        self.Uf = nn.Linear(self.hidden_size, self.hidden_size, bias=True)
        self.Wi = nn.Linear(self.input_size, self.hidden_size, bias=True)
        self.Ui = nn.Linear(self.hidden_size, self.hidden_size, bias=True)
        self.Wo = nn.Linear(self.input_size, self.hidden_size, bias=True)
        self.Uo = nn.Linear(self.hidden_size, self.hidden_size, bias=True)
        self.Wc = nn.Linear(self.input_size, self.hidden_size, bias=True)
        self.Uc = nn.Linear(self.hidden_size, self.hidden_size, bias=True)
        self.activation_g = nn.Sigmoid()
        self.activation_c = nn.Tanh()
    ...
    def forward(self, x, h=None, c=None):
        ...
        for xt in x.unbind(0):
            ft = self.activation_g(self.Wf(xt) + self.Uf(h))
            it = self.activation_g(self.Wi(xt) + self.Ui(h))
            ot = self.activation_g(self.Wo(xt) + self.Uo(h))
            c = ft*c + it*self.activation_c(self.Wc(xt) + self.Uc(h))
            h = ot*self.activation_h(c)
            h_seq.append(h)
            c_seq.append(c)
        h_seq = torch.cat(h_seq, 0)
...
class Embedding(nn.Module):
    def __init__(self, num_embeddings, embedding_dim, padding_idx):
        ...
        self.weight = torch.normal(mean=0, std=1, size=(self.num_embeddings, self.embedding_dim))
